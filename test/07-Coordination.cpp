...
// 兼容性配置
    ...
    // ------------------------------------
    Shader customizedShader("shaders/07/shader.vs", "shaders/07/shader.fs"); // you can name your shader files however you like
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        customizedShader.setMat4("projection", projection); // note: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
